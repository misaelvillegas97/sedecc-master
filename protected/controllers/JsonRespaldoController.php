...
class JsonController extends Controller
{
	...
	public function actionCargo(){
		...
		foreach ($global['categorias'] as $key => $value){
			$global['items'][$value['car_id']] = Yii::app()->db->createCommand("SELECT * FROM min_pregunta WHERE car_id = '".$value['car_id']."' ORDER BY tem_id,pre_enunciado")->query()->readAll();
		}
		...
		foreach ($global['equipos'] as $key => $value){
			$global['items_equipos'][$value['car_id']] = Yii::app()->db->createCommand("SELECT * FROM min_pregunta WHERE car_id = '".$value['car_id']."' ORDER BY tem_id,pre_enunciado")->query()->readAll();
		}
		...
		foreach ($global['instalaciones'] as $key => $value){
			$global['items_instalaciones'][$value['car_id']] = Yii::app()->db->createCommand("SELECT * FROM min_pregunta WHERE car_id = '".$value['car_id']."' ORDER BY tem_id,pre_enunciado")->query()->readAll();
		}
		...
		$global['formularios'] = Yii::app()->db->createCommand("SELECT correlativo_chk_eess, checklist, GROUP_CONCAT(eess_rut) as eess_rut, n_campos, campo,nombre_campos,campos_values,campos_requeridos FROM min_formularios GROUP BY correlativo_chk_eess ORDER BY correlativo_chk_eess ASC")->query()->readAll();
