...
class Jsonprueba2Controller extends Controller
{
	...
	{
        $id_eva = Yii::app()->db->createCommand("SELECT eva_id from min_evaluacion_instalaciones where eess_rut = 96960670")->query()->readAll();
        for ($i=0; $i < count($id_eva) ; $i++) { 
            Yii::app()->db->createCommand("delete from min_respuesta_instalaciones where eva_id =".$id_eva[$i]['eva_id'])->execute();
            Yii::app()->db->createCommand("delete from min_evaluacion_instalaciones where eva_id =".$id_eva[$i]['eva_id'])->execute();
        }
        echo 'finishim';
	}

	public function actionCargoTrabajadores() {
		header('Content-type:application/json;charset=utf-8');
	...
	public function actionCargo(){
		...
		$global['categorias'] = Yii::app()->db->createCommand("SELECT DISTINCT UPPER(car_id) AS car_id FROM min_pregunta WHERE tipo_checklist='trabajadores' and eess_rut = ".$_GET['empresa_logueada']."")->query()->readAll();

		foreach ($global['categorias'] as $key => $value){
			$global['items'][$value['car_id']] = Yii::app()->db->createCommand("SELECT pre_id, eess_rut, pre_enunciado, pre_ponderacion, tem_id, UPPER(car_id) as car_id, critico, tipo_checklist FROM min_pregunta WHERE UPPER(car_id) = '".$value['car_id']."' and eess_rut = ".$_GET['empresa_logueada']." ORDER BY tem_id,pre_enunciado")->query()->readAll();
		}
		//creamos json para mostrar los checklist de cada equipos por empresa
		$global['equipos'] = Yii::app()->db->createCommand("SELECT DISTINCT UPPER(car_id) AS car_id FROM min_pregunta WHERE tipo_checklist='equipos' and eess_rut = ".$_GET['empresa_logueada']."")->query()->readAll();

		foreach ($global['equipos'] as $key => $value){
			$global['items_equipos'][$value['car_id']] = Yii::app()->db->createCommand("SELECT pre_id, eess_rut, pre_enunciado, pre_ponderacion, tem_id, UPPER(car_id) as car_id, critico, tipo_checklist FROM min_pregunta WHERE car_id = '".$value['car_id']."' AND eess_rut = ".$_GET['empresa_logueada']." ORDER BY tem_id,pre_enunciado")->query()->readAll();
		}

		//creamos json para mostrar los checklist de cada instalacion por empresa
		$global['instalaciones'] = Yii::app()->db->createCommand("SELECT DISTINCT UPPER(car_id) AS car_id FROM min_pregunta WHERE tipo_checklist='instalaciones' and eess_rut = ".$_GET['empresa_logueada']."")->query()->readAll();

		foreach ($global['instalaciones'] as $key => $value){
			$global['items_instalaciones'][$value['car_id']] = Yii::app()->db->createCommand("SELECT pre_id, eess_rut, pre_enunciado, pre_ponderacion, tem_id, UPPER(car_id) as car_id, critico, tipo_checklist FROM min_pregunta WHERE car_id = '".$value['car_id']."' AND eess_rut = ".$_GET['empresa_logueada']." ORDER BY tem_id,pre_enunciado")->query()->readAll();
		}
		//obtenemos el rut de la empresa para realizar verificacion de que checklist tiene activo
		// $tmp = Yii::app()->db->createCommand("SELECT DISTINCT act_eess FROM min_check_activo WHERE tipo_checklist='trabajadores' AND act_eess = ".$_GET['empresa_logueada']."")->query()->readAll();
		// for($i=0;$i<count($tmp);$i++){
			//obtenemos los distintos checklist por cada EESS
			$activos[$_GET['empresa_logueada']] = Yii::app()->db->createCommand("SELECT DISTINCT UPPER(act_car) as act_car FROM min_check_activo WHERE act_eess = ".$_GET['empresa_logueada']."  AND tipo_checklist='trabajadores'")->query()->readAll();
		// }
		//obtenemos los nombres de distintos checklist que tenemos en la BD
		// $activos[] = Yii::app()->db->createCommand("SELECT DISTINCT act_car FROM min_check_activo WHERE tipo_checklist='trabajadores' AND act_eess = ".$_GET['empresa_logueada']."")->query()->readAll();
		$global['activos'] = $activos;

		///////////////////activos equipos////////////
		//obtenemos el rut de la empresa para realizar verificacion de que checklist tiene activo
		// $tmp_equipos = Yii::app()->db->createCommand("SELECT DISTINCT act_eess FROM min_check_activo WHERE tipo_checklist='equipos' AND act_eess = ".$_GET['empresa_logueada']."")->query()->readAll();
		// for($j=0;$j<count($tmp_equipos);$j++){
			//obtenemos los distintos checklist por cada EESS
		$activos_equipos[$_GET['empresa_logueada']] = Yii::app()->db->createCommand("SELECT DISTINCT UPPER(act_car) as act_car FROM min_check_activo WHERE act_eess = ".$_GET['empresa_logueada']." AND tipo_checklist='equipos'")->query()->readAll();
		// }
		//obtenemos los nombres de distintos checklist que tenemos en la BD
		// $activos_equipos[] = Yii::app()->db->createCommand("SELECT DISTINCT act_car FROM min_check_activo WHERE tipo_checklist='equipos' AND act_eess = ".$_GET['empresa_logueada']."")->query()->readAll();
		...
		// $tmp_instalaciones = Yii::app()->db->createCommand("SELECT DISTINCT act_eess FROM min_check_activo WHERE tipo_checklist='instalaciones' and act_eess = ".$_GET['empresa_logueada']."")->query()->readAll();
		// for($k=0;$k<count($tmp_instalaciones);$k++){
			//obtenemos los distintos checklist por cada EESS
			$activos_instalaciones[$_GET['empresa_logueada']] = Yii::app()->db->createCommand("SELECT DISTINCT UPPER(act_car) as act_car FROM min_check_activo WHERE act_eess = '".$_GET['empresa_logueada']."' AND tipo_checklist='instalaciones'")->query()->readAll();
		// }
		//obtenemos los nombres de distintos checklist que tenemos en la BD
		// $activos_instalaciones[] = Yii::app()->db->createCommand("SELECT DISTINCT act_car FROM min_check_activo WHERE tipo_checklist='instalaciones' and act_eess = ".$_GET['empresa_logueada']."")->query()->readAll();
		$global['activos_instalaciones'] = $activos_instalaciones;

		//obtenemos los usuarios que se les permite el ingreso a la app
		$global['trabajador'] = Yii::app()->db->createCommand("SELECT tra.eess_rut, tra.tra_rut, tra.tra_evaluador FROM min_trabajador tra join min_eess emp on emp.eess_rut=tra.eess_rut  where emp.eess_estado=1 and tra.eess_rut = ".$_GET['empresa_logueada']."")->query()->readAll();
		$global['formularios'] = Yii::app()->db->createCommand("SELECT correlativo_chk_eess, UPPER(checklist) as checklist, GROUP_CONCAT(eess_rut) as eess_rut, n_campos, campo,nombre_campos,campos_values,campos_requeridos FROM min_formularios where eess_rut = ".$_GET['empresa_logueada']." GROUP BY correlativo_chk_eess ORDER BY correlativo_chk_eess ASC")->query()->readAll();
	...
	public function actionAutocompletar(){
		...
		$eess_logueada = $_GET['empresa_logueada'];
		$global['cargo'] = Yii::app()->db->createCommand("SELECT * FROM min_cargo")->query()->readAll();
		$global['trabajador'] = Yii::app()->db->createCommand("SELECT * FROM min_trabajador WHERE eess_rut = ".$eess_logueada)->query()->readAll();
		...
		$global['faena'] = Yii::app()->db->createCommand("SELECT eess_rut, fae_nombre as eva_faena FROM min_faena WHERE fae_activo = 1 AND eess_rut = ".$eess_logueada." ORDER BY fae_nombre")->query()->readAll();
		...
		$global['patente'] = Yii::app()->db->createCommand("SELECT DISTINCT(eva_patente) as eva_patente FROM min_evaluacion WHERE eess_rut = ".$eess_logueada)->query()->readAll();
		$global['maquinaria'] = Yii::app()->db->createCommand("SELECT eq_maquina,eq_codigo FROM min_equipos WHERE eess_rut = ".$eess_logueada)->query()->readAll();
