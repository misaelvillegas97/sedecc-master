...
if (Yii::app()->controller->usertype() == 1) {
    # code...
    $contador  = Yii::app()->db->createCommand("SELECT COUNT(*) FROM min_evaluacion_instalaciones WHERE eess_rut = ".Yii::app()->user->id)->queryScalar();
}

// Cuando el usuario ingresado es un evaluador
if (Yii::app()->controller->usertype() == 3) {
    # code...
    $contador  = Yii::app()->db->createCommand("SELECT COUNT(*) FROM min_evaluacion_instalaciones WHERE eess_rut = (SELECT eess_rut FROM min_trabajador WHERE tra_rut = ".Yii::app()->user->id.") and eva_evaluador = ".Yii::app()->user->id)->queryScalar();
}

// Cuando el usuario ingresado es admin
if (Yii::app()->controller->usertype() == 2) {
    # code...
    $contador  = Yii::app()->db->createCommand("SELECT COUNT(*) as trab FROM min_evaluacion_instalaciones as E JOIN min_eess as EE ON(E.eess_rut = EE.eess_rut) WHERE EE.eess_estado = 1")->queryScalar();
}
?>
...
<span class="h1" style="margin-top: 20px;">Evaluación instalaciones</span> <span class="text-muted">(<?php echo $contador; ?> en total)</span>
