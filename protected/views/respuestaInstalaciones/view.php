...
?>
<?php
$dir = 'images/mensaje_instalaciones/';
if(isset($_POST['respuesta_instalaciones'])){
	// Guardar respuesta
	$sql = "INSERT INTO min_mensaje_instalaciones(res_id, men_emisor, men_mensaje) VALUES('".$model->res_id."','".Yii::app()->user->id."','".$_POST['respuesta_instalaciones']."')";
	Yii::app()->db->createCommand($sql)->execute();

	// Obtener id respuesta
	$id = Yii::app()->db->createCommand("SELECT MAX(men_id) FROM min_mensaje_instalaciones")->queryScalar();
	...
	// Enviar correo
	$evaluacion = Yii::app()->db->createCommand("SELECT * FROM min_evaluacion_instalaciones WHERE eva_id = '".$model->eva_id."'")->queryRow();
	$email = '';
	...
	';
	$sql = "SELECT * FROM min_mensaje_instalaciones WHERE res_id = '".$model->res_id."' ORDER BY men_creado";
...
if(isset($_POST['aprobarid'])){
	$sql = "UPDATE min_respuesta_instalaciones SET res_seguimiento = 1 WHERE res_id = '".$_POST['aprobarid']."'";
	Yii::app()->db->createCommand($sql)->execute();
	header('Location: index.php?r=evalInstalaciones/view&id='.$model->eva_id);
}
...
<span style='float:right;'>
	...
	$evaluador = Yii::app()->db->createCommand("SELECT eva_evaluador FROM min_evaluacion_instalaciones WHERE eva_id = '".$model->eva_id."'")->queryScalar();
	$eess = Yii::app()->db->createCommand("SELECT eess_rut FROM min_evaluacion_instalaciones WHERE eva_id = '".$model->eva_id."'")->queryScalar();
	...
	';
	?>
	<a class="btn btn-rounded btn-sm btn-primary" href="index.php?r=evalInstalaciones/view&id=<?php echo $model->eva_id;?>">Volver a la evaluación</a>
</span>
...
if((Yii::app()->user->id == $evaluador || Yii::app()->user->id == 'admin') && $model->res_seguimiento != 1){
	if(isset($_POST['plazo_solucion'])){
		...
		Yii::app()->db->createCommand("UPDATE min_respuesta_instalaciones SET res_plazo = '".$model->res_plazo."' WHERE res_id = '".$model->res_id."'")->execute();
	}
...
<?php
$supervisor_nombre = Yii::app()->db->createCommand("SELECT eva_supervisor FROM min_evaluacion_instalaciones WHERE eva_id = '".$model->eva_id."'")->queryScalar();
//$trabajador = Yii::app()->db->createCommand("SELECT tra_rut FROM min_evaluacion_instalaciones WHERE eva_id = '".$model->eva_id."'")->queryScalar();
...
//$respondetodo = Yii::app()->db->createCommand("SELECT tra_rut FROM min_trabajador WHERE eess_rut = '".Yii::app()->db->createCommand("SELECT eess_rut FROM min_evaluacion_instalaciones WHERE eva_id = '".$model->eva_id."'")->queryScalar()."' AND tra_responder_todo = 1 AND tra_rut = '".Yii::app()->user->id."'")->queryScalar();
if(
...
<form method="post" enctype="multipart/form-data" style="margin-bottom:40px;">
	...
	<input required class="form-control" id="respuesta_instalaciones" name="respuesta_instalaciones" type="text" placeholder="Escriba aquí su respuesta (Enter para enviar)">
