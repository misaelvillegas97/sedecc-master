...
<div class="row" style="margin-bottom: 15px;">
  ...
  <?php 
  	//excepción para proimad y apco, puesto que ellos no tienen trabajadores con certificaciones
  if ($u == '83226000' || $u == '79787480') {
...
if ($u == '83226000' || $u == '79787480') {
  ...
  echo '   <div class="col-md-3" style="padding-right: 0px;">
    <div class="col-md-12" align="center" style="padding: 0px; background-color: #FFFFFF;">
      <div style="width: 100%; border-bottom: 2px solid #eeecee;">Seguimiento a Incumplimientos</div>
      <div id="exportdiv5" style="width: 100%;  height: 250px; overflow-y: auto;">
...
<div class="row" style="margin-top: 10px;">
  ...
  $tituloGrafico = '';
  if ($u == '83226000' || $u == '79787480') {
    $tituloGrafico = '% Cumplimiento por Supervisor';
  } else {
    $tituloGrafico = '% Cumplimiento por Jefe de Faena';
  }
  ?>
    <div class="col-md-12" align="center" style="background-color: #FFFFFF;">
      <div style="border-bottom: 2px solid #eeecee;"><?php echo $tituloGrafico; ?></div>
  ...
  AmCharts.makeChart("chartdiv2",
    {
      ...
      "dataProvider": [
        ...
        // if($rows[$i]['porc'] > Yii::app()->params['riesgomedio']) $rows[$i]['color'] = '#4fbd5b'; //color amarillo
          ...
          echo '
          {
            "cargo": "' . str_replace('"', '', trim($rows[$i]['car_descripcion'])) . '",
  ...
  AmCharts.makeChart("chartdiv3",
    {
        ...
        "balloonText": "[[Porcentaje]]%",
          ...
          "balloonFunction": function(item, graph) {
            console.log(item.dataContext);
              ...
              // if (worker['contadorTra']){
                  result = `  <b>Porcentaje cumplimiento: ${worker['Porcentaje']}%</b> <br>
                              
                  
                  `;
                  return result;
              // }
              // <b>N° Trabajadores: ${worker['contadorTra']}</b>
      ...
      "dataProvider": [
        <?php
        //excepción para apco y proimad, cambiar jefe de faena por supervisor
        $campo = '';
        if ($u == '83226000' || $u == '79787480') {
          $campo = 'eva_supervisor';
        } else {
          $campo = 'eva_jefe_faena';
        }
        $rows = Yii::app()->db->createCommand("SELECT " . $campo . ", AVG(eva_cache_porcentaje) as porc FROM min_evaluacion as E JOIN min_eess as EE ON(E.eess_rut = EE.eess_rut) WHERE EE.eess_estado = 1 and " . $campo . " != '' " . $where2 . " " . $where . " GROUP BY " . $campo . " ORDER BY porc ASC")->query()->readAll();
        for ($i = 0; $i < count($rows); $i++) {
          // $rowsContadorTrabajadores = Yii::app()->db->createCommand("SELECT COUNT( DISTINCT(tra_rut) ) FROM min_evaluacion as E WHERE 1 and " . $campo . " != '" . $rows[$i]['eva_jefe_faena'] . "' " . $where2 . " " . $where . "")->queryScalar();
          // $rows[$i]['contadorTra'] = $rowsContadorTrabajadores;
          $rows[$i]['color'] = '#e14d57';
          if ($rows[$i]['porc'] > Yii::app()->params['riesgoalto']) $rows[$i]['color'] = '#f7e523';
        ...
        ?>
        // "contadorTra": "' . $rows[$i]['contadorTra'] . '",
      ]
    }
  );
</script>
...
WHEN (porc >= 0 AND porc <= '" . $limit1 . "') THEN ((0.0294*porc)+1)
WHEN (porc > '" . $limit1 . "' AND porc <= '" . $limit2 . "') THEN ((0.0667*porc)-2.1333)
WHEN (porc > '" . $limit2 . "' AND porc <= 100) THEN ((0.1750*porc)-12.5000) END,1) as nota
