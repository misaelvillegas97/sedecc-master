...
if(isset($_POST['filtro_codigo'])){
	...
	if($_POST['filtro_codigo'] != "") $where_evaluacion.= " AND e.eva_cod_faena= '".$_POST['filtro_codigo']."' ";
	//if($_POST['filtro_nombre'] != "") $where_evaluacion.= " AND e.eva_faena = '".$_POST['filtro_nombre']."' ";
...
<h2 class="page-header">Reporte de Instalaciones</h2>
<h3>Ranking por Instalación</h3>
<!--FILTROS-->
<form method="post">
<div class="row">
	...
	<div class="col-sm-2">
		...
		<select name="filtro_tipo" class="form-control selectpicker " data-live-search="true" onchange="this.form.submit();">
			<?php
			if(Yii::app()->controller->usertype() == 1){
				$rows = Yii::app()->db->createCommand("SELECT distinct eva_tipo FROM min_evaluacion_instalaciones e WHERE eess_rut = '".Yii::app()->user->id."' ")->query()->readAll();
				$rut_eess = Yii::app()->user->id;
			}
			
			else if(Yii::app()->controller->usertype() == 3){
				$eess = Yii::app()->db->createCommand("SELECT DISTINCT(eess_rut) FROM min_trabajador WHERE tra_rut = '".Yii::app()->user->id."'")->queryScalar();
				$rows = Yii::app()->db->createCommand("SELECT DISTINCT(eva_tipo) FROM min_evaluacion_instalaciones e WHERE eess_rut = '".$eess."' ")->query()->readAll();
				$rut_eess = $eess;
			...
			**/
				...
				$rows = Yii::app()->db->createCommand("SELECT DISTINCT(eva_tipo) FROM min_evaluacion_instalaciones e WHERE 1 ")->query()->readAll();
			}
	...
	<div class="col-sm-2">
		...
		<select  name="filtro_codigo" onchange="this.form.submit();" class="form-control selectpicker " data-live-search="true" >
			<option value="">[Todas]</option>
				...
				$rows = Yii::app()->db->createCommand("SELECT * FROM min_faena e WHERE 1 ".$where_evaluacion_2."  ORDER BY fae_nombre")->query()->readAll();
				for($i=0;$i<count($rows);$i++){
					$selected = "";
					if(isset($_POST['filtro_codigo']))
							if($_POST['filtro_codigo'] == $rows[$i]['fae_id']) 
								$selected = "selected";
					echo '<option '.$selected.' value="'.$rows[$i]['fae_id'].'">'.$rows[$i]['fae_nombre'].'</option>';
				}
...
<?php
if(!isset($rut_eess) || empty($rut_eess)){
  header('Location: index.php?r=site/login');
	return;
}
$tematicas = Yii::app()->db->createCommand("SELECT DISTINCT tem_id FROM min_respuesta_instalaciones as r JOIN min_evaluacion_instalaciones as e ON(r.eva_id = e.eva_id) WHERE e.eess_rut = '".$rut_eess."' ")->query()->readAll();
?>

<!-- SEGUNDO REPORTE -->
<?php
$rows2 = Yii::app()->db->createCommand("SELECT *, UPPER(CONCAT(eva_faena)) as completo, AVG(eva_cache_porcentaje) AS promedio, '#000000' as color,count(e.eva_id) as countEvaluaciones FROM min_evaluacion_instalaciones e WHERE 1 ".$where_evaluacion." GROUP BY eva_faena ORDER BY AVG(eva_cache_porcentaje)")->queryAll();
if(count($rows2)==0){
	...
	<div class="col-sm-9">
		<script>
			$(document).ready(function(){
				var chart1 = AmCharts.makeChart("chartdiv0233", {
			    "type": "serial",
			    "precision":1,
			    "theme": "light",
			    "marginRight": 50,
		...
		<div id="chartdiv0233" style="background:#ffffff; height:400px; border:1px solid #dddddd;"></div>
	...
	<div class="col-sm-3">
		...
		$(document).ready(function(){
			var chart2 = AmCharts.makeChart( "tortatrabajador", {
			  ...
			  "titles": [
			  {
				"text": "<?php /*echo $torta['#FF0000']+$torta['#00CC00']+$torta['#FFFF00'];*/?> INSTALACIONES EVALUADAS\nPOR NIVEL DE RIESGO\n"+<?php echo count($rows2)?>+"",
				"size": 11
			  },
			  ],
			  "valueField": "cantidad",
...
<?php 
   ...
   extract($_GET);
    ...
    $k = 0;
  	$sql1 = "SELECT DISTINCT tem_id FROM min_respuesta_instalaciones as r JOIN min_evaluacion_instalaciones as e ON(r.eva_id = e.eva_id) WHERE 1 $where_evaluacion";
				...
				var chart3<?php echo $k;?> = AmCharts.makeChart("item<?php echo $k;?>", {
					...
					<?php
						$sql2 = "SELECT r.res_enunciado, SUM(IF(r.res_respuesta = 'si',1,0)) AS S, SUM(IF(r.res_respuesta = 'no',1,0)) AS N, SUM(IF(r.res_respuesta = 'n/a',1,0)) AS NA
								...
								JOIN min_respuesta_instalaciones as r ON(e.eva_id = r.eva_id)
...
<?php 
	$rows = Yii::app()->db->createCommand("SELECT * FROM min_evaluacion_instalaciones e WHERE 1 ".$where_evaluacion."")->queryAll();
	if(count($rows)==0){
	...
	echo '
	<tr>
		<td><a href="index.php?r=evalIntalaciones/view&id='.$rows[$i]['eva_id'].'" target="_blank">'.$rows[$i]['eva_faena'].'</a></td>
		<td><a href="index.php?r=evalIntalaciones/view&id='.$rows[$i]['eva_id'].'" target="_blank">'.$rows[$i]['eva_fecha_evaluacion'].'</a></td>
		'.$hdrs.'
