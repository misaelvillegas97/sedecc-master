...
?>
	...
	<div class="row" style="margin-bottom: 1em;">
		<div class="col-sm-9" style="float: left;">
	        <section class="panel" style="border-width:0px;  margin: 0px;">
...
<div class="col-md-9" style="padding:0px !important; margin-bottom: 1em; background:#ffffff; border:1px solid #dddddd;">
<?php
	foreach ($cargosEess as $cargo) {
		# Se declara un gráfico por cada Cargo que posea cada empresa.
		$query2 = Yii::app()->db->createCommand("SELECT eva.eess_rut, AVG(eva_cache_porcentaje) AS eva_nota
					FROM min_evaluacion AS eva
					JOIN min_trabajador AS tra
						ON 	eva.tra_rut = tra.tra_rut
					WHERE 	tra.car_id = ".$cargo['car_id']." AND 
							eva.eess_rut = $eess")->query()->readAll();
		// echo json_encode($query2);
		if( $query2[0]['eva_nota'] != NULL && $query2[0]['eva_nota'] > 0) {

		
?>
	<!-- <div class="col-md-3" style="background-color: #FFFFFF; border-radius: 8px; background:#ffffff;  border:1px solid #dddddd;"> -->
	<div class="col-md-3" style="">
		<!-- amCharts javascript code -->
		<script type="text/javascript">
			var graficoCargo<?php echo $cargo['car_id']?> = AmCharts.makeChart("graf_cargo<?php echo $cargo['car_id']?>",
				{
					"dataLoader": {
						"url": "graphsC/reportes_tra_cargo.php?eess_rut=<?php echo $eess ?>&car_id=<?php echo $cargo['car_id'] ?>",
						"format": "json"
					},
					"type": "pie",
					"balloonText": "[[title]]<br><span style='font-size:8pt'><b>[[value]]</b> ([[percents]]%)</span>",
					"fontSize": 10,
					"innerRadius": "40%",
					"groupedAlpha": 0, 
					"labelsEnabled": false,
					...
					"balloonFunction": function(item, graph) {
						...
						if (worker['titulo'] == 'nombre :v') {
							return ;
						}

						return '<b>'+worker["titulo"]+'</b>: '+worker["eva_nota"]+'%';
					},
					"balloon": {
						"showBullet": true
					},
					...
					"titles": [
						{
							"id": "Title-1",
							"size": 12,
							"text": "<?php echo $cargo['car_descripcion']?>"
						}
					]
				}
			);
		...
		<div id="graf_cargo<?php echo $cargo['car_id']?>" style="width: 100%; height: 250px;" ></div>
	</div>
<?php
	} # If que valida que los datos no sean NULL
	# Finaliza el foreach que recorre los cargos por empresa.
}
?>
</div>
<br/>
