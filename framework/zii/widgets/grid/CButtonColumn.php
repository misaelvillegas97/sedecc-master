 ...
 *
 * By default, it will display three buttons, "view", "update" and "trash", which triggers the corresponding
...
class CButtonColumn extends CGridColumn
{
	 ...
	 * These default tokens are recognized: {view}, {update} and {trash}. If the {@link buttons} property
	 ...
	 */
	public $template='{view} {update} {trash}';
	/**
	...
	/**
	 * @var string the label for the trash button. Defaults to "trash".
	 * Note that the label will not be HTML-encoded when rendering.
	 */
	public $trashButtonLabel;
	/**
	 * @var string the image URL for the trash button. If not set, an integrated image will be used.
	 * You may set this property to be false to render a text link instead.
	 */
	public $trashButtonImageUrl;
	/**
	 * @var string a PHP expression that is evaluated for every trash button and whose result is used
	 * as the URL for the trash button. In this expression, you can use the following variables:
	 * <ul>
	 *   <li><code>$row</code> the row number (zero-based)</li>
	 *   <li><code>$data</code> the data model for the row</li>
	 *   <li><code>$this</code> the column object</li>
	 ...
	 * please refer to the {@link http://www.php.net/manual/en/language.expressions.php php manual}.
	 */
	public $trashButtonUrl='Yii::app()->controller->createUrl("trash",array("id"=>$data->primaryKey))';
	/**
	 * @var array the HTML options for the trash button tag.
	 */
	public $trashButtonOptions=array('class'=>'trash');
	/**
	 * @var string the confirmation message to be displayed when trash button is clicked.
	 * By setting this property to be false, no confirmation message will be displayed.
	 * This property is used only if <code>$this->buttons['trash']['click']</code> is not set.
	 */
	public $trashConfirmation;
	/**
	 * @var string a javascript function that will be invoked after the trash ajax call.
	 * This property is used only if <code>$this->buttons['trash']['click']</code> is not set.
	 *
	 * The function signature is <code>function(link, success, data)</code>
	 * <ul>
	 * <li><code>link</code> references the trash link.</li>
	 ...
	 * </ul>
	 * Note that if success is true it does not mean that the trash was successful, it only means that the ajax call was successful.
	 *
	 * Example:
	 * <pre>
	 *  array(
	 *     class'=>'CButtonColumn',
	 *     'aftertrash'=>'function(link,success,data){ if(success) alert("trash completed successfuly"); }',
	 *  ),
	 * </pre>
	 */
	public $aftertrash;
	 ...
	 * please refer to the {@link http://www.php.net/manual/en/language.expressions.php php manual}.
	 *
	 * If the 'buttonID' is 'view', 'update' or 'trash' the options will be applied to the default buttons.
	 *
	...
	/**
	 * Initializes the default buttons (view, update and trash).
	 */
	protected function initDefaultButtons()
	{
		...
		if($this->trashButtonLabel===null)
			$this->trashButtonLabel=Yii::t('zii','trash');
		if($this->viewButtonImageUrl===null)
			$this->viewButtonImageUrl=$this->grid->baseScriptUrl.'/view.png';
		if($this->updateButtonImageUrl===null)
			$this->updateButtonImageUrl=$this->grid->baseScriptUrl.'/update.png';
		if($this->trashButtonImageUrl===null)
			$this->trashButtonImageUrl=$this->grid->baseScriptUrl.'/delete.png';
		if($this->trashConfirmation===null)
			$this->trashConfirmation=Yii::t('zii','¿Realmente desea eliminar?');

		foreach(array('view','update','trash') as $id)
		{
			$button=array(
		...
		if(!isset($this->buttons['trash']['click']))
		{
			if(is_string($this->trashConfirmation))
				$confirmation="if(!confirm(".CJavaScript::encode($this->trashConfirmation).")) return false;";
			else
				$confirmation='';
			...
			if($this->aftertrash===null)
				$this->aftertrash='function(){}';

			$this->buttons['trash']['click']=<<<EOD
function() {
	$confirmation
	var th = this,
	...
	jQuery('#{$this->grid->id}').yiiGridView('update', {
		...
		success: function(data) {
			jQuery('#{$this->grid->id}').yiiGridView('update');
			aftertrash(th, true, data);
		},
		error: function(XHR) {
			return aftertrash(th, false, XHR);
		}
	});
	return false;
}
EOD;
	 ...
	 * This method renders the view, update and trash buttons in the data cell.
